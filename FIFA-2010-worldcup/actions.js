YUI().use('slider','anim',function(Y){
      ...
      var current = 'time',
          data = fifa.data.query.results.row,
          all = data.length,
          maxval = {time: 0,shots: 0,passes: 0,tackles: 0, saves: 0},
          countries = {},
          positions = {},
          names = '';
          for(var i=1;i<all;i++) {
            ...
            var w = what.toLowerCase();
                ...
                for(var i=0;i<all;i++) {
                        ...
                        if(data[i][w] === value) {
                             ...
                             saves += +data[i].saves;
                             names += '<li><button>' + data[i].surname + '<span>'+ 
                                      'Country: '+ data[i].team + ' Position: '+ data[i].position + 
                                      ' Time: '+ data[i].time + ' Shots: '+ data[i].shots + 
                                      ' Passes: '+ data[i].passes + 
                                      ' Tackles: '+ data[i].tackles + 
                                      ' Saves: ' + data[i].saves +
                                      '</span></button></li>';
                ...
                var n = Y.one('#names').prepend('<ul class="hid">' +
                                                 '<li class="summary"><span>' +
                                                  what + ': ' + value + '</span> Time: ' + time + ' ('+ r(time/count)+ ' average) '+
                                                 'Shots ' + shots + ' (' + r(shots/count) + ' average) Passes '+ passes + ' (' + r(passes/count) + ' average) '+
                                                 'Tackles ' + tackles + ' ('+ r(tackles/count)+ ' average) Saves ' + saves + ' (' + r(saves/count) + ' average)' +
                                                 '<button>delete</button>'+
                                                 '<div></div></li>'+ names +'</ul>');

                var an = Y.one('#names ul').setStyle('opacity',0);
             ...
             Y.one('#position').delegate('click',function(){
                    filter('Position',this.get('innerHTML')); 
             },'button');


             Y.one('#countries').delegate('click',function(){
                  ...
                  if(this.get('innerHTML') === 'delete') {
                         ...
                         var myAnim = new Y.Anim({node: an, to: {opacity: 0}, duration: .5});
                             ...
                             myAnim.on('end',function(){
                                    myAnim2 = new Y.Anim({node: an, to: {height: 0}, duration: .2});
                                    myAnim2.run();
                         ...
                         if(this.get('innerHTML') === Y.one('#player').get('innerHTML')) {
                                ...
                                Y.one('#player').set('innerHTML',this.get('innerHTML')).setX(this.getX()+100).setY(this.getY()+10);
                         } 
                  }  
